        ...
        .run(move |cx: &mut App| {
            ...
            let icon = cx.new(|cx| {
                let reader = image::ImageReader::with_format(
                    Cursor::new(
                        cx.asset_source()
                            .load("assets/octopus-gb.png")
                            .unwrap()
                            .unwrap(),
                    ),
                    image::ImageFormat::Png,
                );
                Arc::new(reader.decode().unwrap().into_rgba8())
            });
            cx.insert_global_entity(icon.clone());
            let icon = icon.read(cx).clone();
                ...
                cx.open_window(
                    ...
                    |window, cx| {
                        cx.update_global::<GlobalState, _>(|global, _cx| {
                            ...
                            global.fast_boot = settings.emulator.fast_boot;
                        });
    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let menu = [
            ...
            Menu::new("Playback").items([
                ...
                MenuItem::separator(),
                MenuItem::action("Toggle Fast-forward", actions::playback::ToggleFastForward)
                    .checked(cx.global::<GlobalState>().fast_forward_on),
            ]),
            ...
            Menu::new("Tools").items([
                ...
                MenuItem::action("Embed ROM in PNG", actions::tools::EmbedRomInPng),
                MenuItem::separator(),
            ...
            Menu::new("Help").items([
                MenuItem::action("About", actions::help::OpenAbout),
                MenuItem::separator(),
                MenuItem::action("Luna", actions::help::Luna),
            ]),
        ];
        ...
        div()
                    ...
                    if !ev.is_held {
                        ...
                        if let Some(binding) = bindings.first() {
                            ...
                            if name.starts_with("game") {
                                let button = {
                                    use octopus_core::GameBoyButton::*;
                ...
                using!([weak_self], move |button,
                    ...
                    weak_self
                        .update(cx, |this: &mut MainWindow, cx| {
                            let bindings = gamepad_service.get_bindings_for_button(*button);

                            for binding in bindings.iter() {
                                if gamepad_service
                                    .check_combination(event.id, binding.combination())
                                {
                                    let name = binding.action().name();
                                    if name.starts_with("game") {
                                        let button = {
            ...
            .on_action::<actions::file::OpenRom>(cx.listener(move |this, _a, _window, cx| {
                ...
                let Some(rom_path) = rfd::FileDialog::new()
                    .add_filter(
                        "All supported files",
                        &["gb", "gbc", "zip", "gz", "tar", "rar", "zst", "xz", "png"],
                    )
            ...
            .on_action::<actions::file::CloseRom>(cx.listener(move |_this, _a, _window, _cx| {}))
            ...
            .on_action(
                cx.listener(|_this, _event: &actions::help::OpenAbout, window, cx| {
                        ...
                        cx.global_mut::<WindowMap>().get(&WindowType::About)
                    {
                        ...
                        let window_handle = AboutWindow::open(window, cx).unwrap().into();
                        cx.global_mut::<WindowMap>()
                            .insert(WindowType::About, window_handle);
                    }
                }),
            )
            .on_action(
                cx.listener(|_this, _event: &actions::help::Luna, window, cx| {
                    if let Some(window_handle) = cx.global_mut::<WindowMap>().get(&WindowType::Luna)
                    {
                        ...
                        let window_handle = LunaWindow::open(window, cx).unwrap().into();
                        cx.global_mut::<WindowMap>()
                            .insert(WindowType::Luna, window_handle);
                    }
                }),
            )
    }
...
pub fn reload_settings(cx: &mut App) {
    ...
    *cx.global_mut::<GlobalState>().palette.lock() = palette;
    ...
    settings.input.set_gamepad_binds(cx);
}
